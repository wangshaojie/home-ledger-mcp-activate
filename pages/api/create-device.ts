...
// 创建设备,返 mcp_token
// 入参: { user_id, email, code, device_name }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { user_id, email, code, device_name } = (req.body ?? {}) as {
    user_id?: string;
    email?: string;
    code?: string;
    device_name?: string;
  ...
  const { data, error } = await supabase.rpc('issue_mcp_token_by_user', {
    p_user_id: user_id,
  ...
  if (error) {
    ...
    console.error('[create-device] Supabase RPC 错误:', msg);
    let userMsg = `授权失败: ${msg}`;
    if (msg.includes('验证码已过期')) userMsg = '验证码已过期(超过 5 分钟),请返回上一步重新申请';
    if (msg.includes('5 台')) userMsg = '设备数已达上限(5 台),请先在桌面端吊销旧设备';
    if (msg.includes('设备名')) userMsg = '设备名不合法(1-100 字符)';
    return res.status(400).json({ error: 'create_failed', message: userMsg, raw: msg });
  }
